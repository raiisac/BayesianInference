...
Non-smokers: posterior summary measures For the non-smokers, the
...
c(qbeta(0.025, shape1 = alpha.ns, shape2 = beta.ns), qbeta(0.975, shape1 = alpha.ns, shape2 = beta.ns))
```

The 95% highest posterior density interval is equal to [0.0219, 0.0314].
...
```{r posterior-rr, echo=FALSE, warning = FALSE, message = FALSE, fig.cap = "Posterior distribution of the relative risk.", fig.width= 6, fig.height=4, fig.align = "default", appendix = TRUE}
...
alpha_smoker <- 172
beta_smoker <- 3265
#draw random values from both posteriors ans calculate relative risk
...
we can conclude that there is indeed an association between smoking ans
...
`r unname(quantiles[3])`. We can thus confidently say that smokers have
a statistically significantly higher risk of getting a stroke than non
smokers.
...
(4) Write jags, OpenBugs or Nimble code, to obtain an MCMC samples for
    the above problem.
:::
We use jags to obtain MCMC samples. First, we specify data values, parameters, and our jags model. To run MCMC, we specify the maximum likelihood estimates as initial values and set the number of chains to 4. We initially set the number of chains to 3 but change to 4 as convergence results appear to be better than 3 without compromising much on computation cost. Finally, we simulate 10,000 values from the approximated posterior distribution calculated using MCMC. We do not specify a burn-in period as convergence results are fine without it.  
...
init.list <- list(theta1 = sum(data.list$y1)/length(data.list$y1), #using maximum likelihood estimates 
...
posterior.sim <- coda.samples(model = jagsmodel, variable.names = c("theta+", "theta_"), n.iter = 10000)
...
:::
Diagnostic measures, Gelman-Rubin statistic and visual plot, trace and ACF plots, as well as effective sample size (ESF) are used to assess convergence.All measures point towards proper convergence. The following delineates the results for each aforementioned diagnostic measure: 

1) Gelman-Rubin statistic and visual plot
The Gelman-Rubin statistic for both $$\theta_+$$ and $$\theta_-$$ are equal to 1. The Gelman-Rubin visual plot further illustrates convergence as the median for both $$\theta_+$$ and $$\theta_-$$ approximately convergences to 1.

2) The trace plots of both $$\theta_+$$ and $$\theta_-$$ do not show irregularity with the chains fluctuating around 0.050 for $$\theta_+$$ and 0.027 for $$\theta_-$$. The trace plots appear to be stable and stationary.

3) The overwhelmingly large effective sample size (ESS) for all chains combined (e.g., $$ESS_{\theta_+} \approx 23,576$$; $$ESS_{\theta_-} \approx 24,182$$) indicates sufficient independent samples. Furthermore, all chains have similar similar ESS scores ranging from 5565 to 6482 which indicates consistency across four chains. All four chains combined and separately point towards proper convergence. 

4) ACF plots of both $$\theta_+$$ and $$\theta_-$$ show rapid decay as lags increase which shows that subsequent sample become less and less correlated to the samples before it. This further points towards proper convergence. 
```{r}
# 1) gelman-rubin 
...
effectiveSize(posterior.sim)# all chains
...
for (i in 1:length(chains)) {
  cat("ESS for Chain", i, ":", ess.values[i], "\n")
}
...
:::
The summary measures obtained analytically and from MCMC are similar. The analytical approach generated a mean of 1.9, with a 95% highest density interval (HDI) that ranged from 1.48 to 2.34. Similarly, the MCMC approach yield a mean of 1.89, with a 95% HDI ranging from 1.46 to 2.34. Here, the difference between the two means is only 0.01 points.
...
par(mfrow = c(1,2))
plotPost(random_rr, xlab = "Analytical Derivation", mainColor = "white", border = "black" )
plotPost(relative.risk, xlab = "MCMC Computation", mainColor = "white", border = "black")
mtext("Analytical Derivation vs MCMC Computation of Relative Risk", side = 3, line = 2, adj = 1, cex = 2)
```
...
two vague priors as a massive variance (extremely small precision) for
...
found to converge around the same conclusion. Furthermore, we conducted
...
malformations is ?????0.143????? in the absence of any administered
...
$$α \sim N(0,10000)$$ $$β \sim N(0,10000)$$ All other tuning parameter
...
chains were found to converge around the same conclusion. The
