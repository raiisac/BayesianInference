...
The equal tail credible interval for nonsmokers is equal to [0.0430, 0.05758]
```{r}
alpha_smoker <- 172
beta_smoker <- 3265
c(qbeta(0.025, shape1 = alpha_smoker, shape2 = beta_smoker <- 3265), qbeta(0.975, shape1 = alpha_smoker, shape2 = beta_smoker <- 3265))
```
...
jagsmodel <- jags.model(file = textConnection(model.string), data = data.list, n.chains = 4)

# Step 4: simulate values from the posterior distribution
posterior.sim <- coda.samples(model = jagsmodel, variable.names = c("theta1", "theta2"), n.iter = 10000)
...
The Gelman-Rubin statistic for both $$\theta_+$$ and $$\theta_-$$ is equal to 1, and the Gelman-Rubin visual plot further illustrates convergence as the median for both $$\theta_+$$ and $$\theta_-$$ approximately converges to 1.

2) The trace plots of both $$\theta_+$$ and $$\theta_-$$ do not show irregularity, with the chains fluctuating around a singular point, (e.g., 0.050 for $$\theta_+$$ and 0.027 for $$\theta_-$$), and appear to be stable and stationary.

3) The overwhelmingly large ESS for all chains combined (e.g., $$ESS_{\theta_+} \approx 23,576$$; $$ESS_{\theta_-} \approx 24,182$$) indicates sufficient independent samples. Furthermore, all chains have similar ESS scores ranging from 5565 to 6482, indicating consistency across four chains. All four chains combined and separately point towards proper convergence.

4) ACF plots of both $$\theta_+$$ and $$\theta_-$$ show rapid decay as lags increase, indicating that subsequent samples become less and less correlated with the samples before them. This further points towards proper convergence.
```{r}
# 1) gelman-rubin 
...
plotPost(relative.risk, mainColor = "white", border = "black", xlab = "Relative Risk")
...
:::
The mean attributable risk is 0.466 with 95% HDI equal to [0.337, 0.583]. This means that smoking is a significant contributing factor to stroke cases, with a range of 33.7% to 58.3% and a mean of 46.6%.  

```{r}
attrib.risk <- (relative.risk - 1) / relative.risk
plotPost(attrib.risk, mainColor = "white", border = "black", xlab = "Attributable Risk")
```
