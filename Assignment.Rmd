...
In general, if $Y|\theta \sim Bin(n,\pi)$ is the data model and $\theta \sim Beta(\alpha, \beta)$ is the prior model, then the posterior model will be
...
We draw a random sample of size `r n` from both posterior Beta distributions and calculate the relative risk based on those. Figure \@ref(fig:posterior-rr) shows a histogram of the random sample of the relative risk. If the relative risk is significantly different from one, we can conclude that there is indeed an association between smoking ans stroke. Let's investigate this.

The mean relative risk in our sample is `r round(mean(random_rr),3)` and the variance is `r round(var(random_rr),3)`. The median relative risk is `r unname(quantiles[2])`. The minimal relative risk is our sample is `r round(min(random_rr),3)`, meaning that the probability that the relative risk $<=1$ is equal to zero in our sample! Indeed, 95% of our sample lies between `r unname(quantiles[1])` and `r unname(quantiles[3])`. We can thus confidently say that smokers have a statistically significantly higher risk of getting a stroke than non smokers.
